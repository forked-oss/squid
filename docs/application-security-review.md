...
## 1. [HIGH] Cache Manager reflects attacker-controlled Origin with credentialed CORS letting malicious sites read localhost-reachable manager reports
...
## 2. [HIGH] Heap buffer overflow when appending DNS search path in idnsALookup
...
## 4. [MEDIUM] FTP directory listing XSS via unescaped fallback LIST lines
...
## 5. [MEDIUM] Reflected XSS in URN resolution page with unescaped URN URL and resolver-supplied URLs in generated HTML
