...
## 4. [MEDIUM] Reflected XSS in URN resolution page — unescaped URN URL and resolver-supplied URLs in generated HTML

- **Location:** `src/urn.cc`
- **Attacker:** Proxy client crafting a URN request URL, or attacker controlling the URN resolver response
- **Controlled input:** URN URL in the client request and URLs returned by the external URN resolver
- **Attack path:** `urnHandleReply` interpolates `e->url()` and resolver `u->url` directly into HTML `<TITLE>`, `<H2>`, and `<A HREF>` without `html_quote()`
- **Impact:** JavaScript execution in browsers of users whose traffic passes through the affected Squid instance
